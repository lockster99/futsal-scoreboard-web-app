...
def get_today_fixtures_db(cursor, venue):
    ...
    for row in cursor.fetchall():
        ...
        fixtures.append(Fixture(row[0], competitions[row[2]], row[3], row[4], teams[row[5]], row[6], row[7], teams[row[8]], datetime.strptime(row[10],"%Y-%m-%d %H:%M:%S"), row[11], row[12], row[13], period_configuration))
